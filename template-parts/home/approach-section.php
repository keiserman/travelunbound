<section class="bg-white">
    ...
    if ($faq_query->have_posts()) {
        ...
        while ($faq_query->have_posts()) {
            ...
            if ($thumbnail_url) {
                echo '<img data-faq="image" class="w-full h-full object-cover absolute top-0 opacity-0 pointer-events-none" src="' . esc_url($thumbnail_url) . '" alt="' . esc_attr(get_the_title()) . '">';
            }
            echo '</div>';
        }
        echo '</div>';
