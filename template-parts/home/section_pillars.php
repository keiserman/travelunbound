            ...
            <?php while ($query->have_posts()) : $query->the_post(); ?>
                <div data-pillar="pillar" data-pillar-color="<?php echo get_field('color') ?>" class="padding-global border-b py-6 md:py-12 relative cursor-pointer overflow-hidden">
                    ...
                    <?php if ($image_url = get_field('image')): ?>
                        <img data-pillar="image" class="w-full h-full object-cover absolute top-0 left-0 opacity-0 pointer-events-none" src="<?php echo esc_url($image_url); ?>" alt="<?php esc_attr(get_the_title()); ?>">
                    <?php endif; ?>
                </div>
            <?php endwhile; ?>
        </div>
