<section>
    <div class="container grid gap-2">
        <h3>in today's socially connected world, the nonstop online chatter, and constant state of flux can be overwhelming. we navigate the fast-changing landscape giving you</h3>
        <div class="grid grid-cols-2 gap-4 items-stretch">
            <div class="relative">
                <img src="" alt="" class="aspect-[4/5] bg-white">
                <div class="text-sm vertical-text absolute bottom-0 -right-6">photo credit: daniela federici</div>
            </div>
            <div class="flex flex-col justify-between">
                <div class="flex flex-col justify-start gap-2">
                    <h3>the time and the space to fully immerse yourself in your travel experience.</h3>
                    <a href="/" class="button primary self-start	">enquire</a>
                </div>
                <div class="flex gap-2 flex-col pl-20">
                    <p class="font-semibold">Crafted for the modern traveller</p>
                    <p>we understand the hectic nature of modern life where multitasking is almost second mature; constant demands on our time and attention, the relentless struggle for work/life balance, competing priorities and an ever-growing to-do list. we save you time and work to your pace.</p>
                </div>
            </div>
        </div>
    </div>
</section>